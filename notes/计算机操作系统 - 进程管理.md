...
管道是通过调用 pipe 函数创建的，进程在内核开辟一块缓冲区，并用两个文件描述符fd[0] 指向管道读端，fd[1] 指向写端。
...
也称为命名管道，所以去除了管道只能在父子进程中使用的限制，本质上是一个设备文件，所以需要对它有打开，关闭等操作。
...
它是一个计数器，用于为多个进程提供对共享数据对象的访问，例如读者写者问题里记录读者数量，等读者数量为0才可写。
